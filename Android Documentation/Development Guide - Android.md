...
In ooVoo a video call is technically named as *“conference”*. Each conference is identified by a unique *“Conference ID”*. To connect two or more users of your application in a video call, you simply put those users in the same ooVoo conference by having them joining conference using the same Conference ID.
...
The ooVoo SDK provides you an object through which you can access most of the functionality provided by SDK. This is the *entry point* into the SDK:
...
If you got `ConferenceState.Joined` then technically you are already in the conference. You will start getting events about other participants asyncronously. Also, at this point, you can start audio/video.
...
This returns a Boolean value and will let you know if this capability is enabled for you current application.
You can send messages to either all participants in a conference or only to one specific participant. To send a message to ALL participants of a conference:
...
•	Conference status change: e.g., participant join or leave the conference.
•	Participant status change: e.g., participant’s video stream turned on/off.
•	Connection quality status change: e.g., reporting connection quality issues.
